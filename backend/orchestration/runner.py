...
from backend.orchestration.state import get_live_state, initial_state
...
def latest_state(run_id: str) -> dict[str, Any]:
    st = dict(get_live_state(run_id))
    st.update(_latest_state.get(run_id) or {})
    if st and any(k in st for k in ("code", "requirements", "test_report")):
        return st
    try:
        ...
        for art in artifacts:
            ...
            if key and key not in st and art.get("content"):
                try:
                    import json
                    st[key] = json.loads(art["content"])
                except Exception:
                    st[key] = art["content"]
        if st:
            _latest_state[run_id] = st
    except Exception:
        pass
    return st
...
def _execute(run: dict[str, Any], project: dict[str, Any], options: dict[str, Any]) -> None:
    run_id = run["id"]
    repositories.update_run(run_id, status="RUNNING", current_stage="START")
    ...
    try:
        ...
        if _stop_flags.get(run_id):
            ...
            repositories.update_run(run_id, status="STOPPED", current_stage=result.get("current_stage"))
            return
        ...
        repositories.update_run(run_id, status="FAILED", error_message=str(exc), completed_at=utc_now())


def continue_run(run_id: str, requested_stage: str | None = None, updates: dict[str, Any] | None = None) -> dict[str, Any]:
    state_updates: dict[str, Any] = {"requested_stage": requested_stage, "stop_requested": False}
    if updates:
        state_updates.update(updates)
    result = resume_workflow(run_id, state_updates)
    _latest_state[run_id] = result
    status = result.get("final_status") or "WAITING"
    repositories.update_run(
        run_id,
        status=status if status != "PENDING" else "WAITING",
        current_stage=result.get("current_stage"),
        rework_count=result.get("rework_count", 0),
        completed_at=utc_now() if status in {"COMPLETED", "MANUAL_INTERVENTION_REQUIRED"} else None,
    )
...
def start_continue_thread(run_id: str, requested_stage: str | None = None, updates: dict[str, Any] | None = None) -> None:
    thread = threading.Thread(target=_execute_continue, args=(run_id, requested_stage, updates), daemon=True)
    thread.start()


def _execute_continue(run_id: str, requested_stage: str | None = None, updates: dict[str, Any] | None = None) -> None:
    try:
        continue_run(run_id, requested_stage, updates)
    except Exception as exc:
        logger.error(f"workflow resume failed: {exc}", extra={"run_id": run_id, "error_category": "WORKFLOW_ERROR"})
        emit(run_id, EventType.PIPELINE_FAILED.value, str(exc), "WORKFLOW", status="FAILED")
        repositories.update_run(run_id, status="FAILED", error_message=str(exc), completed_at=utc_now())
